...
type AssetItem = {
  kind: "ASSET";
  sourceKind: "ASSET" | "CREATIVE_PACKAGE";
  assetId: string;
  type: "IMAGE" | "VIDEO";
  url: string;
  width: number | null;
...
type PlaylistItem = AssetItem | CollectionItem;
...
export default function SignagePlayer({ deviceId }: { deviceId: string }) {
  ...
  const currentKey = !currentItem
    ...
    : currentItem.kind === "ASSET"
      ? `${currentIndex}:${currentItem.assetId}:${currentItem.url}`
      : `${currentIndex}:collection:${currentItem.collectionId}`;
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      if (currentItem.kind === "ASSET") {
        ...
        const delivered = await sendProof(deviceId, token, [event]).catch(() => false);
        if (!delivered) {
  ...
  if (connection === "UNPAIRED") {
    return (
      <PlayerMessage
        ...
        action={<Link href="/player" className="rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-emerald-950 hover:no-underline">Open pairing</Link>}
      />
    );
  }
  ...
  if (!playlist || !currentItem) {
    return (
      <PlayerMessage
        eyebrow={connection === "OFFLINE" ? "Connection interrupted" : "Screen connected"}
        title={connection === "OFFLINE" ? "Running without a fresh playlist." : "Waiting for an active campaign."}
        body={playlist ? `${playlist.device.label} · ${playlist.device.name}` : "Contacting the G-SPAN control plane…"}
      />
    );
  }
  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden bg-black text-white">
      ...
      <div className="pointer-events-none absolute right-4 top-4 flex items-center gap-2 rounded-full bg-black/55 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/70 backdrop-blur">
        <span className={`h-1.5 w-1.5 rounded-full ${connection === "ONLINE" ? "bg-emerald-400" : "bg-amber-400"}`} />
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-black p-8 text-center text-white">
      <div className="max-w-xl">
        <div className="text-xs font-semibold uppercase tracking-[0.25em] text-emerald-400">{eyebrow}</div>
...
function CollectionWidget({ item, token }: { item: CollectionItem; token: string }) {
  const [feed, setFeed] = useState<{ collection: { name: string }; items: Array<{ id: string; title: string; body: string | null }> } | null>(null);
  ...
  return (
    <div className="grid h-full w-full place-items-center bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.22),transparent_36%),#070a08] p-[5vw]">
      <div className="w-full max-w-6xl">
        <div className="text-[1.2vw] font-semibold uppercase tracking-[0.24em] text-emerald-400">Live collection</div>
        <h1 className="mt-[1vw] text-[5vw] font-semibold leading-none tracking-tight">{feed?.collection.name ?? "Updating…"}</h1>
        <div className={`mt-[3vw] grid gap-[1.5vw] ${item.renderMode === "GRID" ? "grid-cols-2" : "grid-cols-1"}`}>
          {feed?.items.slice(0, item.renderMode === "TICKER" ? 1 : 6).map((entry) => (
            <div key={entry.id} className="rounded-[1.5vw] border border-white/10 bg-white/5 p-[2vw]">
              <div className="text-[2.4vw] font-semibold">{entry.title}</div>
              {entry.body ? <div className="mt-[0.6vw] text-[1.4vw] text-zinc-300">{entry.body}</div> : null}
            </div>
          ))}
        </div>
      </div>
